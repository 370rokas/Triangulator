...
class Edge {
  ...
  protected:
	unsigned short	ptIdx[2];

  public:
	Edge()	{ data = 0; mark = 0; ptIdx[0] = ptIdx[1] = 0xFFFF; }
	Edge* Rot();
	Edge* invRot();
	Edge* Sym();
	Edge* Onext();
	...
	bool		isLMarked() const { return (mark & 1) != 0; }
	bool		isRMarked() const { return (mark & 2) != 0; }
...
class Mesh {
  private:
	...
	Edge *MakeEdge(bool);
	Edge *MakeEdge(Point2d*, Point2d*, bool);
  ...
  public:
	...
	void CreateIndexedMesh( void* vbuf, int& nVert, int vstride, 
							unsigned short* ibuf, int& nInd );

	static Real s_VertexWeldTreshold;
...
inline bool Edge::isConstrained()
{
