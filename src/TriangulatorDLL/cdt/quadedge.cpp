...
void Mesh::DeleteEdge(Edge* e)
{
	...
	if (startingEdge->QEdge() == e->QEdge()) {
		assert(!"Mesh::DeleteEdge: attempting to delete starting edge");
		// try to recover:
		startingEdge = (e != e->Onext()) ? e->Onext() : e->Dnext();
		cdtAssert(startingEdge->QEdge() != e->QEdge());
	}
...
// whose origin is the new site.
{
	...
	if (!hasLeft && !hasRight) {
		assert(!"InsertSite: edge does not have any faces");
		return NIL(Edge);
	}
	...
	if (!insideLeft && !insideRight) {
		//assert(!"InsertSite: point not in a face adjacent to edge");
		return NIL(Edge);
	}	                    
	...
	if (!insideLeft) {
		assert(!"InsertSite: point is not to the left of edge");
		return NIL(Edge);
	}

	// x should be strictly inside the left face:
	if (OnEdge(x, e->Onext()) || OnEdge(x, e->Dprev())) {
		assert(!"InsertSite: point is not strictly inside face");
		return NIL(Edge);
	}
...
// Inserts a constraining edge into a CDT.
{
	...
	if (ea == NIL(Edge) || eb == NIL(Edge)) {
		//assert(!"InsertEdge: could not insert endpoints of edge");
		return;
	}

	startingEdge = ea;
	if ((ea = Locate(aa)) == NIL(Edge)) {
		assert(!"InsertEdge: could not locate an endpoint");
	    return;
	}
	if (!(aa == ea->Org2d()))
	    ea = ea->Sym();
	if (!(aa == ea->Org2d())) {
		assert(!"InsertEdge: point a is not an endpoint of ea");
		return;
	}

	if (aa == bb) {
		assert(!"InsertEdge: both ends map to same vertex");
		return;
	}
	...
	while (ea->Org() != last) {
		...
		do {
			...
			ea = ea->Onext();
			if (ea == t) {
				assert(!"InsertEdge: infinite loop");
				return;
			}
		} while (TRUE);
...
// In the event of failure, we pass the search to BruteForceLocate.
{
	...
	while (TRUE) {
		...
		// fi          rst, examine the endpoints for coincidence:
		if (x == e->Org2d())
		...
		assert(!"Locate: I'm stuck and I can't move on");
		return BruteForceLocate(x);
	}	
}
...
// Same as Locate, but uses a brute force exhaustive search.
{
	...
	assert(!"Locate: Could not locate using brute force");
	return NIL(Edge);
}
...
Real Mesh::s_VertexWeldTreshold = EPS;
