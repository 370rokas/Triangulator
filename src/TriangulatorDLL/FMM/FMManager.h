...
// - разрешение на проверку корректного доступа к памяти
//#define FMM_BOUNDSCHECKING_ENABLE

#define FMM_LOGFILENAME "FMManager.log"
#define FMM_DEFAULTMEMSIZE (1024*1024*1024)
#define FMM_PHYSMEM_PERCENT 75
#define FMM_MAXHANDLERS	(1024*1024)
...
public:
	// -------------------- data definitions -----------------------------------------
	#ifdef FMM_LOCKING_ENABLE
		// - разрешено ли распределение блоков в режиме защиты?
		static bool isLockingEnable;	
	#endif
	#ifdef FMM_BOUNDSCHECKING_ENABLE
		// - здоровый массив выделенности памяти (бит на байт)
		static unsigned char MemoryMask[(FMM_DEFAULTMEMSIZE+7)>>3];
	#endif
	...
	unsigned FMMHASHS(unsigned size) 
	{
		...
		//unsigned h = (unsigned)(atan((double)(size>>4)/3000.0)*800.0);
